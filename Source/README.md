...
**SylvieCooldownTracker.PostExposeData**:
```csharp
public override void PostExposeData()
{
    base.PostExposeData();
    Scribe_Collections.Look(ref cooldownTrackers, "cooldownTrackers", LookMode.Value, LookMode.Value);
    
    if (cooldownTrackers == null)
    {
        cooldownTrackers = new Dictionary<int, int>();
    }
}
```
...
2. 每个使用静态数据的组件提供 `Clear()` 或 `ClearCache()` 方法
3. 在游戏重置时调用清理
...
**问题**: 保存的 Pawn 引用可能在加载后变为无效（死亡、被销毁）。
