...
### 事件系统
游戏开始后约 2 游戏小时（5000 ticks，约83秒实际时间），自动触发奴隶商人事件：
- 商人带来希尔薇（作为囚犯）
- 支付 100 白银即可收留她
- 事件仅触发一次，确保希尔薇的独特性
...
| 套装 | 黑色连衣裙、终末地服装、和服、旗袍、女仆装、重装女仆、学生服、西装、典雅服装、新春花嫁、护士服 |
...
| 典雅服装 | 全局工作速度 +5% |
| 重装女仆 | 移动速度 -10%（提供更高防护） |
| 护士服 | 主动技能：紧急治疗（包扎全身伤口，昏迷约42秒（约1游戏小时），冷却约83秒（约2游戏小时）） |
...
- **编译警告修复**：
  - 修复 CS8602 空引用警告
- **修复冷却动画组件渲染层级问题**：
  - 将 Sweat 组件的渲染层级调整为 baseLayer 61（位于胡子和头发之间）
  - 解决 Sweat 组件被头发遮挡的问题
- **修复冷却动画组件缩放和位置计算**：
